# src/fixed_sources.py
"""固定优质源配置 - 纯数据，由数据库管理"""

# 固定央视频道源配置
# 每个频道可指定多个备选URL，系统启动时自动同步到数据库
# auto_optimize=True 表示允许系统在发现更优源时自动替换（默认）
# auto_optimize=False 表示永久锁定，永不自动替换
CCTV_FIXED_SOURCES = {
    "CCTV-1": ["http://45.192.97.170:8880/play/1.m3u8"],
    "CCTV-2": ["http://45.192.97.170:8880/play/2.m3u8"],
    "CCTV-3": ["http://69.30.245.50/live/cctv3.m3u8"],
    "CCTV-4": ["http://69.30.245.50/live/cctv4.m3u8"],
    "CCTV-5": ["http://69.30.245.50/live/cctv5.m3u8"],
    "CCTV-5+": ["http://45.192.97.170:8880/play/6.m3u8"],
    "CCTV-6": ["http://69.30.245.50/live/cctv6.m3u8"],
    "CCTV-7": ["http://69.30.245.50/live/cctv7.m3u8"],
    "CCTV-8": ["http://69.30.245.50/live/cctv8.m3u8"],
    "CCTV-9": ["http://69.30.245.50/live/cctv9.m3u8"],
    "CCTV-10": ["http://69.30.245.50/live/cctv10.m3u8"],
    "CCTV-11": ["http://45.192.97.170:8880/play/12.m3u8"],
    "CCTV-12": ["http://45.192.97.170:8880/play/13.m3u8"],
    "CCTV-13": ["http://45.192.97.170:8880/play/14.m3u8"],
    "CCTV-14": ["http://45.192.97.170:8880/play/15.m3u8"],
    "CCTV-15": ["http://45.192.97.170:8880/play/16.m3u8"],
    "CCTV-16": ["http://183.11.239.36:808/hls/169/index.m3u8"],
    "CCTV-17": ["http://45.192.97.170:8880/play/17.m3u8"],
    "CCTV4欧洲": ["http://45.192.97.170:8880/play/18.m3u8"],
    "CCTV4美洲": ["http://45.192.97.170:8880/play/19.m3u8"],
}

# 是否启用固定源
ENABLE_FIXED_SOURCES = True
