...
def parse_m3u(content: str) -> List[Dict[str, str]]:
    ...
    while i < len(lines):
        ...
        if line.startswith("#EXTINF"):
            ...
            if match: group_title = match.group(1)
            match = re.search(r'tvg-id="([^"]+)"', line)
            if match: tvg_id = match.group(1)
            match = re.search(r'tvg-logo="([^"]+)"', line)
            if match: tvg_logo = match.group(1)
            parts = line.split(",")
            ...
            if i + 1 < len(lines) and not lines[i + 1].startswith("#"):
                ...
                if url.startswith(("http://", "https://", "rtmp://", "rtsp://")):
                    channels.append({"name": name, "url": url, "group_title": group_title, "tvg_id": tvg_id, "tvg_logo": tvg_logo})
            i += 2
        else:
            i += 1
    return channels
...
def apply_alias_to_channels(channels):
    matcher = get_alias_matcher()
    if not matcher: return channels
    for ch in channels:
        original = ch["name"]
