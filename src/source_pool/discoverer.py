...
class SourceDiscoverer:
    ...
    async def discover(self, db=None, filter_domestic: bool = True, force_refresh: bool = False) -> Dict[str, List[RawSource]]:
        ...
        for src in new_sources:
            grouped.setdefault(src.channel_name, []).append(src)
    ...
    def get_statistics(self) -> dict:
        stats = {"total": len(self.pool), "pending": 0, "verified": 0, "failed": 0, "promoted": 0}
        for s in self.pool.values():
            if s.status == SourceStatus.PENDING: stats["pending"] += 1
            elif s.status == SourceStatus.VERIFIED: stats["verified"] += 1
            elif s.status == SourceStatus.FAILED: stats["failed"] += 1
            elif s.status == SourceStatus.PROMOTED: stats["promoted"] += 1
        return stats
