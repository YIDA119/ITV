...
# 目标分类
TARGET_CATEGORIES = ["广播", "韩国女团", "音乐吧", "热门歌曲", "动感舞曲", "广场舞"]
CATEGORY_DISPLAY_NAME = {
    "广播": "📻 网络电台", "韩国女团": "🎤 韩国女团", "音乐吧": "🎵 音乐吧",
    "热门歌曲": "🔥 热门歌曲", "动感舞曲": "🎧 动感舞曲", "广场舞": "💃 广场舞",
}
CATEGORY_KEYWORDS = {
    "广播": ["广播", "电台", "fm", "am", "动听"],
    "韩国女团": ["歌团", "女团", "kpop"],
...
async def probe_channel_quick(url: str, session: aiohttp.ClientSession) -> bool:
    ...
    try:
        async with session.get(url, timeout=PROBE_TIMEOUT, allow_redirects=True, headers=headers) as resp:
            ...
            error_keywords = ["<html", "<!doctype", "403", "forbidden", "access denied", "404", "not found",
                              "请勿滥用", "该资源暂不可用"]
            ...
            if data.startswith(b'#EXTM3U') or b'#EXTINF' in data:
                return True
            video_signatures = [b'\x00\x00\x00\x18ftyp', b'\x00\x00\x00\x1cftyp', b'\x1a\x45\xdf\xa3',
                                b'\x47\x40\x00', b'FLV', b'RIFF']
        ...
        if ',' in line and (current_category in result or current_province):
            ...
            if len(parts) == 2:
                ...
                if url.startswith(('http://', 'https://')):
                    if current_province:
                        province_channels.setdefault(current_province, [])
    ...
    async with HttpClient.session_context() as session:
        async def check_one(name, url):
            async with semaphore:
                ...
                return (name, url) if ok else None
